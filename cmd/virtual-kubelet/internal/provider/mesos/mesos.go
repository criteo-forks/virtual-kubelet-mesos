...
import (
	...
	"github.com/virtual-kubelet/mesos/scheduler"
)
...
// MesosProvider implements the virtual-kubelet provider interface and stores pods in memory.
type MesosProvider struct { // nolint:golint
	nodeName           string
...
// MesosConfig contains a mesos virtual-kubelet's configurable parameters.
type MesosConfig struct { //nolint:golint
...
func loadConfig(providerConfig string) (config MesosConfig, err error) {
	...
	if q.Cmp(resource.MustParse(minCPUCapacity)) == -1 {
		return config, fmt.Errorf("CPU value %v is less than the minimum %v", config.CPU, minCPUCapacity)
	}
	...
	if q.Cmp(resource.MustParse(minMemoryCapacity)) == -1 {
		return config, fmt.Errorf("Memory value %v is less than the minimum %v", config.Memory, minMemoryCapacity)
	}
	if q, err = resource.ParseQuantity(config.Pods); err != nil {
		return config, fmt.Errorf("Invalid pods value %v", config.Pods)
	}
	if q.Cmp(resource.MustParse(minPodCapacity)) == -1 {
		return config, fmt.Errorf("Pod value %v is less than the minimum %v", config.Pods, minPodCapacity)
	}
...
func (p *MesosProvider) CreatePod(ctx context.Context, pod *v1.Pod) error {
	...
	return p.mesosScheduler.AddPod(pod)
}

// UpdatePod accepts a Pod definition and updates its reference.
func (p *MesosProvider) UpdatePod(ctx context.Context, pod *v1.Pod) error {
...
func (p *MesosProvider) DeletePod(ctx context.Context, pod *v1.Pod) (err error) {
...
func (p *MesosProvider) GetPod(ctx context.Context, namespace, name string) (pod *v1.Pod, err error) {
...
func (p *MesosProvider) GetContainerLogs(ctx context.Context, namespace, podName, containerName string, opts api.ContainerLogOpts) (io.ReadCloser, error) {
...
func (p *MesosProvider) RunInContainer(ctx context.Context, namespace, name, container string, cmd []string, attach api.AttachIO) error {
...
func (p *MesosProvider) GetPodStatus(ctx context.Context, namespace, name string) (*v1.PodStatus, error) {
...
func (p *MesosProvider) GetPods(ctx context.Context) ([]*v1.Pod, error) {
...
func (p *MesosProvider) ConfigureNode(ctx context.Context, n *v1.Node) {
	ctx, span := trace.StartSpan(ctx, "mesos.ConfigureNode") //nolint:ineffassign
	defer span.End()
...
// within Kubernetes.
func (p *MesosProvider) nodeConditions() []v1.NodeCondition {
...
// within Kubernetes.
func (p *MesosProvider) nodeAddresses() []v1.NodeAddress {
...
// within Kubernetes.
func (p *MesosProvider) nodeDaemonEndpoints() v1.NodeDaemonEndpoints {
...
func (p *MesosProvider) GetStatsSummary(ctx context.Context) (*stats.Summary, error) {
	var span trace.Span
